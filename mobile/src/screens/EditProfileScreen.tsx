...
export default function EditProfileScreen() {
  ...
  const findUserByUsername = useUserStore((state) => state.findUserByUsername);
  ...
  const pickImage = async () => {
    console.log('📸 EditProfile: Checking photo library permission...');
    ...
    if (currentStatus === 'undetermined') {
      console.log('📸 EditProfile: Requesting permission (will show iOS prompt)...');
      const { status: newStatus } = await ImagePicker.requestMediaLibraryPermissionsAsync();
      finalStatus = newStatus;
      console.log('📸 EditProfile: Permission request result:', finalStatus);
    }

    // If permission is denied, show alert directing to Settings
    if (finalStatus === 'denied') {
      console.log('❌ EditProfile: Permission denied - showing settings alert');
      Alert.alert(
    ...
    if (finalStatus === 'granted') {
      console.log('✅ EditProfile: Permission granted - launching picker...');
      try {
  ...
  const takePhoto = async () => {
    console.log('📷 EditProfile: Checking camera permission...');

    // First check current permission status
    const { status: currentStatus } = await ImagePicker.getCameraPermissionsAsync();
    console.log('📷 EditProfile: Current camera permission status:', currentStatus);

    let finalStatus = currentStatus;

    // If not determined yet, request permission (this shows the iOS system prompt)
    if (currentStatus === 'undetermined') {
      console.log('📷 EditProfile: Requesting camera permission (will show iOS prompt)...');
      const { status: newStatus } = await ImagePicker.requestCameraPermissionsAsync();
      finalStatus = newStatus;
      console.log('📷 EditProfile: Camera permission request result:', finalStatus);
    }

    // If permission is denied, show alert directing to Settings
    if (finalStatus === 'denied') {
      console.log('❌ EditProfile: Camera permission denied - showing settings alert');
      Alert.alert(
    ...
    if (finalStatus === 'granted') {
      console.log('✅ EditProfile: Camera permission granted - launching camera...');
      try {
