...
export default function NotificationsScreen() {
  ...
  const [expandedCategories, setExpandedCategories] = useState<Set<string>>(new Set(["friends", "memories", "activity", "reminders", "other"]));
  ...
  const getNotificationCategoryInfo = (type: NotificationType) => {
    switch (type) {
      ...
      case "weekly_reminder":
        return { category: "reminders", title: "Reminders" };
      default:
  ...
  const handleNotificationPress = (notification: Notification) => {
    ...
    const memoryId = notification.data?.memoryId;
    ...
    switch (notification.type) {
      ...
      case "comment_liked":
      case "tagged_in_memory":
        // Navigate to memory detail if we have a memoryId
        if (memoryId) {
          navigation.navigate("MemoryDetail", { memoryId });
        }
        break;
      case "friend_request_accepted":
        // Could navigate to friends tab or friend's profile
  ...
  const renderNotification = ({ item: notification }: { item: Notification }) => {
    ...
    const hasNavigation = notification.data?.memoryId && [
      "memory_liked",
      "memory_comment",
      "comment_reply",
      "comment_liked",
          ...
          <View className="flex-1">
            ...
            <View className="flex-row items-center">
              ...
              {hasNavigation && (
                <View className="flex-row items-center ml-2">
                  <Text className="text-xs text-blue-500">Tap to view</Text>
                  <Ionicons name="chevron-forward" size={12} color="#3B82F6" style={{ marginLeft: 2 }} />
                </View>
              )}
            </View>
            ...
            {isFriendRequest && requestId && (
              <View className="flex-row mt-2" style={{ gap: 8 }}>
                ...
                >
                  <Text className="text-gray-700 text-center text-sm font-medium">Decline</Text>
                </Pressable>
              </View>
            )}
          </View>
