...
if (hasNativeModule()) {
  ...
  console.warn("   This is normal in Expo Go or development without native build");
  isModuleAvailable = false;
}
...
const checkModuleAvailable = (): boolean => {
  if (!isModuleAvailable || !Purchases) {
    // Use warn instead of error to avoid breaking the app
    return false;
  }
  return true;
};
...
export const initializeRevenueCat = async (userId?: string): Promise<void> => {
  ...
  if (!checkModuleAvailable()) {
    console.warn("⚠️ RevenueCat module not available - app will continue without premium features");
    return;
  }
...
export const getOfferings = async (): Promise<any> => {
  ...
  if (!checkModuleAvailable()) {
    console.warn("⚠️ Cannot fetch offerings - module not available");
    return null;
  }
...
export const getMonthlyPackage = async (): Promise<any> => {
  ...
  if (!checkModuleAvailable()) {
    console.warn("⚠️ Cannot get package - module not available");
    return null;
  }
  ...
  try {
    ...
    const monthlyPackage = offering.availablePackages.find((pkg: any) => pkg.identifier === PACKAGE_ID);
    ...
    const monthlyTypePackage = offering.availablePackages.find((pkg: any) => pkg.packageType === "MONTHLY");
    ...
    const productPackage = offering.availablePackages.find((pkg: any) => pkg.product?.identifier === PRODUCT_ID);
...
export const purchasePackage = async (
  ...
  if (!checkModuleAvailable()) {
    console.warn("⚠️ Cannot purchase - RevenueCat module not available (normal in development)");
    return {
      success: false,
...
export const restorePurchases = async (): Promise<{
  ...
  if (!checkModuleAvailable()) {
    console.warn("⚠️ Cannot restore - module not available");
    return {
      success: false,
      error: "Subscription system not available. Please check your internet connection and try again.",
    };
 ...
 */
export const setupCustomerInfoListener = (callback: (customerInfo: any) => void): void => {
  if (!checkModuleAvailable()) {
    return;
  }
 ...
 */
export const removeCustomerInfoListener = (callback: (customerInfo: any) => void): void => {
  if (!checkModuleAvailable()) {
    return;
  }
