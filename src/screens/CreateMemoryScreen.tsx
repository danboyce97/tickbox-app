...
export default function CreateMemoryScreen() {
  ...
  const addMemory = useMemoryStore((state: { addMemory: any }) => state.addMemory);
  const isPremium = useSubscriptionStore((state: { isPremium: any }) => state.isPremium);

  // Flow state
  const isEditMode = route.params?.mode === "edit";
  const editMemoryId = route.params?.memoryId;
  const memories = useMemoryStore((state: { memories: any }) => state.memories);
  const existingMemory = isEditMode && editMemoryId ? memories.find((m: { id: any; }) => m.id === editMemoryId) : null;
  const [showUpgradeModal, setShowUpgradeModal] = useState(false);

  // Add premium checks
  const canCreateTicket = useUserStore((state: { canCreateTicket: any }) => state.canCreateTicket);
  ...
  const incrementTicketCount = useUserStore((state: { incrementTicketCount: any }) => state.incrementTicketCount);
  ...
  const userMemories = user ? memories.filter((m: { userId: any; }) => m.userId === user.id) : [];
  ...
  const [flowType, setFlowType] = useState<FlowType | null>(
    route.params?.mode === "upload"
      ? "upload"
      : route.params?.mode === "manual"
        ? "digital"
        : existingMemory
          ? existingMemory.type === "uploaded"
            ? "upload"
            : "digital"
          : null,
  );
  ...
  const showImagePickerOptions = (type: "upload" | "cover" | "memory") => {
    if (type === "upload") {
      Alert.alert("Upload Ticket Photo", "Choose how you'd like to add your ticket photo:", [
        { text: "Take Photo", onPress: () => handleCameraLaunch(type) },
        { text: "Choose from Library", onPress: () => handleImagePicker(type) },
        { text: "Cancel", style: "cancel" },
      ]);
    } else {
      Alert.alert("Add Photo", "Choose how you'd like to add your photo:", [
        { text: "Take Photo", onPress: () => handleCameraLaunch(type) },
        { text: "Choose from Library", onPress: () => handleImagePicker(type) },
        { text: "Cancel", style: "cancel" },
      ]);
  ...
  const handleImagePicker = async (type: "upload" | "cover" | "memory") => {
    ...
    if (!result.canceled && result.assets) {
      if (type === "memory" && result.assets.length > 1) {
        ...
        const newPhotos = result.assets.map((asset: { uri: any; }) => asset.uri);
        ...
        if (newPhotos.length > remainingSlots) {
          Alert.alert(
            "Limit Reached",
            `You can only add ${remainingSlots} more photos. Only the first ${remainingSlots} photos were added.`,
          );
        }
  ...
  const removeMemoryPhoto = (index: number) => {
    const newPhotos = formData.memoryPhotos.filter((_: any, i: number) => i !== index);
  ...
  const handleSave = async () => {
    ...
    try {
      const memoryData = {
        ...
        seatingInfo:
          !formData.generalAdmission && (formData.entrance || formData.block || formData.row || formData.seat)
            ? {
  ...
  const renderFlowSelection = () => (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      <View className="flex-1 px-6 py-8">
        ...
        <TickBoxCard style={{ marginBottom: 16 }} noPadding children={undefined}>
          <Pressable
            onPress={() => {
              setFlowType("upload");
              setCurrentStep("upload");
        ...
        <TickBoxCard noPadding children={undefined}>
          <Pressable
            onPress={() => {
              setFlowType("digital");
              setCurrentStep("details");
      ...
      >
        <ScrollView className="flex-1 px-6" keyboardShouldPersistTaps="handled">
          <View className="py-6">
            ...
            {/* Upload Area */}
            <TickBoxCard style={{ marginBottom: 24 }} children={undefined}>
            ...
            {formData.uploadedImage && (
              <GradientBackground style={{ borderRadius: 16 }}>
                <Pressable onPress={() => setCurrentStep("details")} style={{ paddingVertical: 16 }}>
                  <Text className="text-white text-center font-semibold text-lg">Continue to Details</Text>
                </Pressable>
              </GradientBackground>
            )}
          </View>
        ...
        <ScrollView className="flex-1 px-6" keyboardShouldPersistTaps="handled">
          <View className="py-6">
            ...
            {detailStep === 0 && (
              <View>
                {/* Event Details */}
                <TickBoxCard style={{ marginBottom: 16 }} children={undefined}>
                  <Text style={{ color: colors.text }} className="text-lg font-semibold mb-4">
                    Event Information
                  </Text>
                  ...
                  <View className="mb-4">
                    ...
                    <TextInput
                      ...
                      onChangeText={(value: any) => setFormData({ ...formData, title: value })}
                      style={{
                        borderWidth: 1,
                        borderColor: colors.border,
                        borderRadius: 12,
                  ...
                  <View className="mb-4">
                    ...
                    <TextInput
                      value={formData.time}
                      onChangeText={(value: any) => setFormData({ ...formData, time: value })}
                      style={{
                        borderWidth: 1,
                        borderColor: colors.border,
                        borderRadius: 12,
                  ...
                  <View className="mb-4">
                    ...
                    <TextInput
                      ...
                      onChangeText={(value: any) => setFormData({ ...formData, location: value })}
                      style={{
                        borderWidth: 1,
                        borderColor: colors.border,
                        borderRadius: 12,
                    ...
                    <View className="flex-row space-x-3">
                      <View className="flex-1">
                        <TextInput
                          ...
                          onChangeText={(value: any) => setFormData({ ...formData, price: value })}
                          style={{
                            borderWidth: 1,
                            borderColor: colors.border,
                            borderRadius: 12,
                      ...
                      >
                        <Text style={{ color: colors.text, fontWeight: "600" }}>{formData.currency}</Text>
                ...
                {flowType === "digital" && (
                  <TickBoxCard style={{ marginTop: 16 }} children={undefined}>
                    <Text style={{ color: colors.text }} className="text-lg font-semibold mb-4">
                    ...
                    <View className="mb-4">
                      <View className="flex-row items-center justify-between mb-3">
                        ...
                        <Switch
                          ...
                          onValueChange={(value: any) => setFormData({ ...formData, generalAdmission: value })}
                      ...
                      <View className="space-y-4">
                        ...
                        <View>
                          ...
                          <TextInput
                            ...
                            onChangeText={(value: any) => setFormData({ ...formData, entrance: value })}
                            style={{
                              borderWidth: 1,
                              borderColor: colors.border,
                              borderRadius: 12,
                        ...
                        <View>
                          ...
                          <TextInput
                            ...
                            onChangeText={(value: any) => setFormData({ ...formData, block: value })}
                            style={{
                              borderWidth: 1,
                              borderColor: colors.border,
                              borderRadius: 12,
                        ...
                        <View className="flex-row space-x-3">
                          <View className="flex-1">
                            ...
                            <TextInput
                              value={formData.row}
                              onChangeText={(value: any) => setFormData({ ...formData, row: value })}
                              style={{
                                borderWidth: 1,
                                borderColor: colors.border,
                                borderRadius: 12,
                          ...
                          <View className="flex-1">
                            ...
                            <TextInput
                              value={formData.seat}
                              onChangeText={(value: any) => setFormData({ ...formData, seat: value })}
                              style={{
                                borderWidth: 1,
                                borderColor: colors.border,
                                borderRadius: 12,
            ...
            {detailStep === 1 && (
              <View>
                ...
                {/* Memory Photos */}
                <TickBoxCard children={undefined}>
                  <Text style={{ color: colors.text }} className="text-lg font-semibold mb-4">
                  ...
                  {formData.memoryPhotos.length > 0 && (
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-4">
                      {formData.memoryPhotos.map((photo: any, index: any) => (
                        <View key={index} className="mr-3 relative">
                          ...
                          <Pressable
                            onPress={() => {
                              const newPhotos = formData.memoryPhotos.filter((_: any, i: any) => i !== index);
                              setFormData({ ...formData, memoryPhotos: newPhotos });
                            }}
                            style={{
                              position: "absolute",
              ...
              <View>
                <TickBoxCard style={{ marginBottom: 16 }} children={undefined}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(value: any) => setFormData({ ...formData, description: value })}
                    style={{
                      borderWidth: 1,
                      borderColor: colors.border,
                      borderRadius: 12,
                ...
                <TickBoxCard children={undefined}>
                  <View className="flex-row items-center justify-between">
                    ...
                    <Switch
                      ...
                      onValueChange={(value: any) => setFormData({ ...formData, showOnFeed: value })}
                      trackColor={{ false: colors.border, true: colors.primary }}
                      thumbColor="white"
                    />
                  </View>
        ...
        <ScrollView className="flex-1 px-6" keyboardShouldPersistTaps="handled">
          <View className="py-6">
            ...
            {isQRProtectionActive() && (
              <TickBoxCard style={{ marginBottom: 16, backgroundColor: `${colors.primary}20` }} children={undefined}>
                <View className="flex-row items-start">
                  ...
                  <View className="flex-1">
                    ...
                    <Text style={{ color: colors.textSecondary }} className="text-sm">
                      This memory will be temporarily private since the event is in the future. It will become visible
                      to friends after the event date.
                    </Text>
                  </View>
                </View>
              </TickBoxCard>
  ...
  const renderCurrencyModal = () => (
    ...
    >
      ...
      >
        <Pressable onPress={(e: { stopPropagation: () => any; }) => e.stopPropagation()}>
          <View
            style={{
                  ...
                  <View className="flex-row items-center justify-between">
                    <View className="flex-row items-center">
                      ...
                      <View>
                        ...
                        <Text style={{ color: colors.textSecondary, fontSize: 14 }}>{currency.name}</Text>
                      </View>
                    </View>
